...
export default function ItAssets() {
  ...
  const { data: response, isLoading, isError } = useQuery({
    queryKey: ["assets", "it"],
    queryFn: () => getAssetSummary(IT_CATEGORY_ID),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      // FIX: correct invalidateQueries syntax for React Query v5
      queryClient.invalidateQueries({ queryKey: ["assets", "it"] });
      setOpen(false);
    },
    onError: (err) => {
  ...
  if (isLoading) {
    return (
      <Grid item xs={12}>
        <Card>
          <CardContent>
            <Typography>Loading IT Assets...</Typography>
          </CardContent>
        </Card>
      </Grid>
    );
    ...
    <Grid item xs={12}>
      <Card>
        <CardContent>
          ...
          {/* HEADER */}
          <Box sx={{ mb: 2 }}>
            <Typography variant="h6">IT Assets</Typography>
          </Box>
            ...
            <TableBody>
              {paginatedData.map((sub) => (
                <TableRow key={sub.id} hover onClick={() => handleOpen(sub)}>
                  ...
                  <TableCell>{sub.operatingSystem}</TableCell>
                  ...
                  <TableCell>
                    {/* FIX: edit icon was missing stopPropagation — clicking it
                        also fired the row's onClick (handleOpen), causing double fetch */}
                    <IconButton
                      color="primary"
                      onClick={(e) => {
                        e.stopPropagation();
